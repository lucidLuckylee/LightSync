...
// This is a lightsync addition to allow the creation of a merkle tree over all block headers.
// Represent a block header using 3 felts and hash them together with pedersen.
// Doing this allows to use the block header hashes as leaves in a Merkle tree.
// WARN: There might be weaker security assumptions for the Merkle tree (1. because we use Pedersen and 2. reduce the domain of inputs)
//       which enables an attacker to proof the inclusion of a block that was not part of the batch easier than finding a preimage of a regular Pedersen hash.
func pedersen_hash_block_header{hash_ptr: HashBuiltin*}(block_header: BlockHeader) -> (pedersen_hash: felt) {
    const BASE = 2 ** 32;
    ...
    return (pedersen_hash=pedersen_block_hash); 
}
